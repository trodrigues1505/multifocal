...
async function pushToFirestore() {
  ...
  try {
    ...
    await setDoc(ref, { tasks: state.tasks, cats: state.cats, nextId: state.nextId });
  } catch(e) {
...
function initDefaults() {
  state.cats = [
    { id:"work",     label:"Trabalho", color:"#6C47FF" },
    { id:"personal", label:"Pessoal",  color:"#10A97A" },
    { id:"learn",    label:"Estudos",  color:"#2563EB" }
  ];
  const t = today();
  state.tasks = [
    ...
    { id:2, title:"Enviar proposta ao cliente", desc:"", status:"blocked", priority:"urgent", due:addDays(t,2), cat:"work", deps:[1], subs:[], comments:[
      { id:"c1", text:"Cliente pediu ajuste no escopo — aguardando e-mail", createdAt: now()-7200000 }
    ...
    { id:5, title:"Preparar apresentação", desc:"Slides reunião da semana", status:"review", priority:"high", due:addDays(t,3), cat:"work", deps:[1], subs:[], comments:[], tags:["#slides","#reunião"], recurrence:"", note:"", timeEst:3, created:now()-20000000 },
...
function getFiltered() {
  ...
  const td = today(); const we = weekEnd();
...
function render() {
  ...
  if (!items.length) {
    list.innerHTML = `<div class="empty"><i class="ti ti-checklist"></i><p>Nenhuma tarefa por aqui.</p></div>`;
    return;
  }
  const td = today();
  list.innerHTML = items.map(t => {
    ...
    return `<div class="task-card${done?" done":""}${selId===t.id?" selected":""} p-${prio}" onclick="App.selTask(${t.id})">
      ...
      <div class="task-meta">
        ...
        <span class="meta-icon" style="color:var(--text3)">${esc(cat.label)}</span>
...
function toggleDone(id) {
  ...
  if (t.status === "done") {
    ...
    if (t.recurrence && t.due) {
      ...
      // create a new copy
      const newTask = { ...JSON.parse(JSON.stringify(t)), id: state.nextId++, status:"todo", due: next, created: now(), subs:[], comments:[] };
      state.tasks.push(newTask);
    }
  }
  persist();
  render();
  if (selId === id) renderDetail(id);
  toast(t.status === "done" ? "Tarefa concluída ✓" : "Tarefa reaberta");
}
function nextRecur(recurrence, fromDate) {
  const n = recurrence === "daily" ? 1 : recurrence === "weekly" ? 7 : 30;
...
function renderDetail(id) {
  ...
  const subItems = subs.map(s => `
    <div class="sub-item">
      <div class="sub-check${s.done?" done":""}" onclick="App.toggleSub(${id},'${s.id}')" title="Toggle">
        <i class="ti ti-check"></i>
      </div>
      <div class="sub-item-main">
        <span class="sub-item-title${s.done?" done":""}">${esc(s.title)}</span>
        <span class="sub-item-date">${s.done && s.completedAt ? "✓ " + fmtDateTime(s.completedAt) : "adicionada " + fmtDateTime(s.createdAt)}</span>
      </div>
      <button class="sub-del" onclick="App.delSub(${id},'${s.id}')" title="Remover"><i class="ti ti-x"></i></button>
    </div>`).join("");

  const depItems = deps.map(d => {
    const f = state.tasks.find(x => x.id === d);
    if (!f) return "";
    return `<div class="dep-item">
      ...
      <button onclick="App.removeDep(${id},${d})" title="Remover"><i class="ti ti-x"></i></button>
    </div>`;
  }).join("");

  const commentItems = comments.map(c => `
    <div class="comment-item">
      <button class="comment-del" onclick="App.delComment(${id},'${c.id}')" title="Excluir"><i class="ti ti-x"></i></button>
      ${esc(c.text)}
      <div class="comment-meta">${fmtDateTime(c.createdAt)}</div>
    </div>`).join("");

  const tagsVal = (t.tags || []).join(" ");

  $("detail-panel").innerHTML = `
    <div class="dp-head">
      <div class="dp-title">${esc(t.title)}</div>
      <button class="icon-btn" onclick="App.selTask(${id})" title="Fechar"><i class="ti ti-x"></i></button>
    </div>
    <div class="dp-body">
      ...
      <div class="dp-field"><label>Tags</label>
        <input type="text" value="${esc(tagsVal)}" placeholder="#tag1 #tag2" onchange="App.updTags(${id},this.value)">
      </div>
      ...
      <!-- SUBTASKS -->
      <div>
        <div class="dp-section-label">Subtarefas / Histórico de ações
          ${subs.length ? `<span style="font-weight:400;color:var(--text3);margin-left:4px">${subDone}/${subs.length}</span>` : ""}
        </div>
        ${subs.length ? `<div class="sub-prog-wrap">
          <div class="sub-prog-row"><span>${subPct}% das ações concluídas</span></div>
      ...
      <div>
        ...
        <div class="comment-list">${commentItems || `<p style="font-size:11px;color:var(--text3)">Nenhum comentário ainda.</p>`}</div>
...
function upd(id, field, val) {
  ...
  if (!t) return;
  t[field] = val;
  persist(); render(); renderDetail(id);
}
...
function updTags(id, val) {
  const t = state.tasks.find(x => x.id === id);
  if (!t) return;
  t.tags = val.trim().split(/\s+/).filter(Boolean).map(tag => tag.startsWith("#") ? tag : "#"+tag);
  persist(); render(); renderDetail(id);
}
...
function toggleSub(tid, sid) {
  ...
  persist(); render(); renderDetail(tid);
}
...
function addDep(id) {
  ...
  if (!t) return;
  if (!t.deps) t.deps = [];
  if (!t.deps.includes(depId)) { t.deps.push(depId); persist(); render(); renderDetail(id); }
}
...
function delTask(id) {
  ...
  persist(); render(); toast("Tarefa excluída");
}
...
function openModal(eid) {
  ...
  const cs = $("m-cat");
  cs.innerHTML = state.cats.map(c => `<option value="${c.id}">${esc(c.label)}</option>`).join("");
  if (t) cs.value = t.cat;
...
function saveTask() {
  ...
  if (editId) {
    ...
    t.title = title; t.desc = $("m-desc").value;
    ...
    state.tasks.push({
      id: state.nextId++, title, desc: $("m-desc").value,
      status: $("m-status").value, priority: $("m-prio").value,
      due: $("m-due").value, cat: $("m-cat").value,
      deps:[], subs:[], comments:[], tags,
...
function openNewCat() {
  ...
  if (!name?.trim()) return;
  const colors = ["#6C47FF","#10A97A","#2563EB","#F59500","#E53935","#0EA5E9","#D946EF"];
...
function renderDashboard() {
  ...
  const catDist = state.cats.map(c => ({
  ...
  // Status donut
  const statusDist = [
    { label:"A fazer",    color:"#2563EB", count: all.filter(t=>t.status==="todo").length },
    { label:"Andamento",  color:"#F59500", count: all.filter(t=>t.status==="doing").length },
    { label:"Revisão",    color:"#6C47FF", count: all.filter(t=>t.status==="review").length },
    { label:"Bloqueada",  color:"#E53935", count: all.filter(t=>t.status==="blocked").length },
    { label:"Concluída",  color:"#10A97A", count: done.length },
  ].filter(s => s.count > 0);
  const statusTotal = all.length || 1;

  // Weekly completion (last 7 days)
  const weekData = Array.from({length:7}, (_,i) => {
    const d = addDays(td, i-6);
    return { day: DAYS_PT[new Date(d+"T12:00:00").getDay()], isToday: d===td,
      count: done.filter(t => { if(!t.due) return false; return t.due === d; }).length };
  });
  ...
  const prioDist = [
    ...
    { label:"Média",   color:"#2563EB", count: active.filter(t=>t.priority==="medium").length },
    { label:"Baixa",   color:"#C0BFBD", count: active.filter(t=>t.priority==="low").length },
  ];
  ...
  const donutSlices = statusDist.map(s => {
    const frac   = s.count / statusTotal;
    ...
    return `<circle cx="40" cy="40" r="30" fill="none" stroke="${s.color}" stroke-width="10"
      stroke-dasharray="${dash.toFixed(2)} ${(CIRCUM-dash).toFixed(2)}"
  ...
  $("dashboard-panel").innerHTML = `
    ...
    <div class="dash-chart-row">
      ...
      <div class="dash-chart-box">
        <h4>Conclusões — últimos 7 dias</h4>
        <div class="week-bars">
          ${weekData.map(d => `
    ...
    <div class="dash-section">
      <h3>Tarefas atrasadas (${overdue.length})</h3>
      <div class="overdue-list">
        ${overdue.slice(0,8).map(t => `
          <div class="overdue-item">
            <span class="overdue-title">${esc(t.title)}</span>
            <span class="overdue-date">${fmtDate(t.due)}</span>
            <span class="badge ${STATUS_CLS[t.status]||"b-todo"}" style="font-size:10px">${STATUS_LABEL[t.status]||""}</span>
          </div>`).join("")}
      </div>
    </div>` : ""}
  `;
...
function exportJSON() {
  ...
  const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "foco-backup.json"; a.click();
  toast("Exportado ✓");
}
...
function importJSON(e) {
  ...
  r.onload = ev => {
    try {
      ...
      if (d.tasks && d.cats) { state = d; persist(); render(); toast("Importado ✓"); }
...
// ── Persist ───────────────────────────────────────────────────────────────────
function persist() {
  saveLocal();          // always local as backup
  pushToFirestore();    // also cloud if logged in
}
...
// ── Keyboard ──────────────────────────────────────────────────────────────────
document.addEventListener("keydown", e => { if (e.key === "Escape") { closeModal(); closeDashboard({ target: $("dashboard-overlay") }); } });
